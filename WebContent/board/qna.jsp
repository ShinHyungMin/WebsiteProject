			...
			<ul class="pagination">
				...
				<c:forEach var="i" begin="0" end="4">
					<c:if test="${(i+startNum) <= lastNum}">
						<a sytle="color:${(page==(i+startNum))?'orange':''} !important" href="?p=${i+startNum}&column=${param.column}&keyword=${param.keyword}">${i+startNum}</a>
					</c:if>
				</c:forEach>				
				</li>
				<li>
					<c:if test="${startNum+4 < lastNum}">
					<a href="?p=${startNum+5}&column=${param.column}&keyword=${param.keyword}">다음</a>
					</c:if>
